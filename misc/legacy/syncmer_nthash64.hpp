...
// - csyncmer_compute_fused_rescan_branchless_64_count() - scalar RESCAN count-only
// - csyncmer_compute_simd_multiwindow_64() - AVX2 SIMD multi-window (requires csyncmer_fast.h)
...
static inline uint64_t csyncmer_legacy_nthash64_rotl7(uint64_t x) {
    return (x << 7) | (x >> 57);
}

static inline void csyncmer_legacy_nthash64_make_f_rot(size_t S, uint64_t f_rot[4]) {
...
static inline void csyncmer_legacy_nthash64_init_ascii_hash_table(uint64_t table[256]) {
...
static inline void csyncmer_legacy_nthash64_init_ascii_to_idx(uint8_t table[256]) {
    memset(table, 0, 256);
    table['A'] = table['a'] = 0;
    table['C'] = table['c'] = 1;
    table['T'] = table['t'] = 2;
 ...
 */
static inline size_t csyncmer_compute_fused_rescan_branchless_64_count(
    const char* sequence,
    size_t length,
    size_t K,
    size_t S,
    ...
    csyncmer_legacy_nthash64_init_ascii_to_idx(IDX_ASCII);
    csyncmer_legacy_nthash64_make_f_rot(S, f_rot);
    ...
    for (size_t j = 0; j < S; ++j) {
        hash = csyncmer_legacy_nthash64_rotl7(hash) ^ F_ASCII[seq[j]];
    }
    ...
    for (size_t i = 1; i < window_size; ++i) {
        hash = csyncmer_legacy_nthash64_rotl7(fw) ^ F_ASCII[seq[i + S - 1]];
...
static inline size_t csyncmer_compute_simd_multiwindow_64(
    ...
    if (num_kmers < 8) {
        return csyncmer_compute_fused_rescan_branchless_64_count(
    ...
    uint64_t hash = 0;
    for (size_t j = 0; j < S; ++j) {
        hash = csyncmer_legacy_nthash64_rotl7(hash) ^ F_ASCII[raw_seq[j]];
    }
    ...
    for (size_t i = 1; i < init_size; ++i) {
        hash = csyncmer_legacy_nthash64_rotl7(fw) ^ F_ASCII[raw_seq[i + S - 1]];
    ...
    for (; k + 8 <= num_kmers; k += 8) {
        ...
        for (; hash_count < end_hash; ++hash_count) {
            hash = csyncmer_legacy_nthash64_rotl7(fw) ^ F_ASCII[raw_seq[hash_count + S - 1]];
